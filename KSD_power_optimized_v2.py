...
# - KSD logic matches Type-I script (U-statistic, empirical centering, grad correction)
# - RBF kernel choice: GaussianKernel(l = sqrt(d) * 0.2)
# ============================================================
...
# grad wrt theta (matches Type-I code; fast path for Gaussian kernel)
@partial(jit, static_argnames=("kernel", "score"))
...
class KSDStatistic(TestStatistic[P_], Generic[P_]):
    def __init__(self, kernel: Kernel, null: Type[UnnormalizedDist]):
        ...
        self.null = null
    def __call__(self, rng: KeyArray, theta_hat: P_, ys: Array) -> Array:
        # Use U-statistic to match Type-I script
...
def gaussian_fixed_scale(scale: Union[float, Scalar]):
    ...
    class GaussianFixedScale(SampleableDist[Array, Array], NormalizedDist[Array], ExpFamilyDist):
        ...
        @staticmethod
        def natural_parameter_inverse(eta_val: Array) -> Array:
            # mean = eta * s^2  (return shape matches get_params: [mean])
            mean = eta_val[0] * (s**2)
...
# RBF kernel lengthscale rule (requested): l = sqrt(d) * 0.2
def make_kernel_for_dim(d: int) -> GaussianKernel:
    return GaussianKernel(l=float(np.sqrt(d) * 0.2))   #TODO same as in the type 1 error
        ...
        for n in n_list:
            ...
            for mu in iterator:
                ...
                for _ in range(runs):
                    ...
                    # 3) KSD parametric bootstrap (stat uses U-stat to match Type-I script)
