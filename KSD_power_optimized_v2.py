...
from jax import Array, grad, jacfwd, jit, vmap, tree_util, lax
...
# Generic (fallback) Stein core using AD — matches Type-I code: jacfwd(jacfwd)
...
# ---- Exact scalar core and its theta-gradient (for gradient correction) ----
@partial(jit, static_argnames=("kernel", "score"))
def h_scalar(kernel: Kernel, score: ScoreFunc, theta: Array, x: Array, xp: Array) -> Array:
    ...
    # Hessian trace w.r.t x then y (matches Type-I script)
    hess_trace = jacfwd(jacfwd(kernel, argnums=0), argnums=1)(x, xp).trace()
...
# ===============
# KSD statistic (configurable U or V)
# ===============
class KSDStatistic(TestStatistic[P_], Generic[P_]):
    def __init__(self, kernel: Kernel, null: Type[UnnormalizedDist], stat: Literal["u", "v"] = "u"):
        self.kernel = kernel
        self.null = null
        assert stat in ("u", "v")
        self.stat = stat
    def __call__(self, rng: KeyArray, theta_hat: P_, ys: Array) -> Array:
        # U for your ~KSD; V for the parametric KSD
        if self.stat == "u":
            return u_stat(self.kernel, self.null.score_with_params, theta_hat, ys)
        else:
            return v_stat(self.kernel, self.null.score_with_params, theta_hat, ys)
    @property
    def name(self) -> str:
        return f"ksd-{self.stat}"
...
def gaussian_fixed_scale(scale: Union[float, Scalar]):
    ...
    class GaussianFixedScale(SampleableDist[Array, Array], NormalizedDist[Array], ExpFamilyDist):
        ...
        def natural_parameter(params: Array) -> Array:
            mean = params[0]
            return jnp.array([mean / (s**2)], dtype=jnp.float32)
        @staticmethod
        def natural_parameter_inverse(eta_val: Array) -> Array:
            mean = eta_val[0] * (s**2)
            return jnp.array([mean], dtype=jnp.float32)
        @staticmethod
        def sufficient_statistic(x: Array) -> Array:
            return jnp.concatenate([x, x**2], axis=0)
        @staticmethod
...
def KSD_test(
    ...
    Preindexes bootstrap indices on host; matches Type-I script details.
    """
    n = X.shape[0]
...
def _KSD_test_preindexed(
    ...
    def one_bootstrap(idx: Array, k_est: KeyArray) -> Array:
        ...
        # Centered core on ORIGINAL X, then subselect [idx][:,idx]
        ...
        # Gradient correction (matches Type-I code)
    ...
    crit = jnp.array(jnp.quantile(T_b_vals, 1.0 - level))
    return {
        "crit": crit,
        "T_obs": T_obs,
        "reject": T_obs > crit,
...
# RBF kernel lengthscale rule: l = sqrt(d)
def make_kernel_for_dim(d: int) -> GaussianKernel:
    return GaussianKernel(l=float(np.sqrt(d)))  # tune c separately if desired  #TODO
...
def _sample_gmm(key: KeyArray, n: int, d: int, mu: float, sigma: float = 1.0) -> Array:
    """
    Two-component symmetric GMM with equal weights and covariance σ^2 I_d.
    Component means are ±μ * 1_d (the same μ in **every** coordinate).
    """
    key_u, key_z = jax.random.split(key)
    signs = 2 * jax.random.bernoulli(key_u, 0.5, shape=(n,)) - 1  # ±1 per sample
    mean_rows = (mu * signs).astype(jnp.float32)[:, None] * jnp.ones((n, d), dtype=jnp.float32)
    z = jax.random.normal(key_z, shape=(n, d))
    return (mean_rows + sigma * z).astype(jnp.float32)
...
def ksd_parametric_reject_one(key: KeyArray, X: Array, *, d: int, B: int = 200, level: float = 0.05, parallel: ParallelMode = 128):
    # Parametric bootstrap KSD should use the V-statistic
    kernel = make_kernel_for_dim(d)
    MVN = mvn_full_family_chol(d)
    estimator = KSDAnalyticEstimator(kernel, MVN)
    stat      = KSDStatistic(kernel, MVN, stat="v")
...
def power_gmm_vs_bfm(
    *,
    ...
    """
    For each (d, n, μ): simulate X ~ GMM_μ with component means ±μ·1_d and run on the SAME X:
      - KSD (~U composite bootstrap)
      - KSD (parametric bootstrap; V-stat)
    ...
    for d in d_list:
        ...
        stat_ksd_V       = KSDStatistic(kernel, MVN, stat="v")  # for parametric KSD
        ...
        for n in n_list:
            ...
            for mu in iterator:
                ...
                rows += [
                    ...
                    dict(test="KSD (parametric bootstrap, V)", d=d, n=n, mu=float(mu), power=float(p_ksd_pb), se=se_ksd_pb, B=B_param, alpha=level),
            ...
            print("KSD param (V) power:", np.array(powers_ksd_param))
