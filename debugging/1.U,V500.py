...
def mean_ksd_statistic_over_runs(
    ...
    for _ in range(runs):
        ...
        dist = null_class(0.0, 1.0)          # μ=0, σ=1
        ys = dist.sample(subkey, n=n)

        # KSD U- und V-Statistik mit den echten Parametern
        u_val = u_stat(kernel, null_class.score_with_params, THETA_TRUE, ys)
        v_val = v_stat(kernel, null_class.score_with_params, THETA_TRUE, ys)
    ...
    mean_u = float(jnp.mean(u_vals))
    std_u  = float(jnp.std(u_vals))
    mean_v = float(jnp.mean(v_vals))
    std_v  = float(jnp.std(v_vals))
    ...
    plt.title(f"U-Stat Histogram (n={n}, runs={runs}, μ=0, σ=1)")
    plt.xlabel("U")
    ...
    plt.title(f"V-Stat Histogram (n={n}, runs={runs}, μ=0, σ=1)")
    plt.xlabel("V")
    plt.ylabel("Frequency")
    plt.grid(True, alpha=0.3)
    plt.legend()
...
# ----  (n=5000, runs=500) ----
mean_u, std_u, mean_v, std_v = mean_ksd_statistic_over_runs(
    rng=key,
    n=5000,
    runs=500,
    kernel=kernel,
...
print(f"U-Stat mean over {500} runs: {mean_u:.6f} (std: {std_u:.6f})")
print(f"V-Stat mean over {500} runs: {mean_v:.6f} (std: {std_v:.6f})")
