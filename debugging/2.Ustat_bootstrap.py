...
def bootstrap_u_tilde_values(
    ...
    # θ̂ auf Originaldaten
    rng, sub_est = jax.random.split(rng)
    theta_hat = estimator(sub_est, ys)


    score_fn = null_class.score_with_params
    _h_fn = h_fn_from_score(score_fn, kernel)
    grad_h_fn_hat = jax.tree_util.Partial(grad_h_fn, theta_hat)

    def one_bootstrap(k: jax.Array) -> jax.Array:
        k_idx, k_est = jax.random.split(k)
        n = ys.shape[0]
        idx = jax.random.choice(k_idx, n, shape=(n,), replace=True)
        ys_star = ys[idx]
        theta_star = KSDAnalyticEstimator(kernel, null_class)(k_est, ys_star)

        h_fn_star = jax.tree_util.Partial(_h_fn, theta_star)
        grad_h_fn_star = jax.tree_util.Partial(grad_h_fn, theta_star)
    ...
    keys = jax.random.split(rng, B)
    T_b_vals = batch_vmap(one_bootstrap, keys, batch_size=10, progress=False)
    return T_b_vals, theta_hat


def run_u_bootstrap_experiment(
    rng: jax.Array,
    *,
    n: int = 5000,
    runs: int = 100,
    ...
    U_obs_all, crit_all, reject_all = [], [], []

    for r in range(runs):
        rng, k_sample, k_boot = jax.random.split(rng, 3)


        ys = null_class(loc, scale).sample(k_sample, n)

        # Bootstrap (tilde-KSD) & θ̂
        T_b_vals, theta_hat = bootstrap_u_tilde_values(k_boot, ys, kernel, null_class, B)


        U_obs = u_stat(kernel, null_class.score_with_params, theta_hat, ys)


        crit = jnp.quantile(T_b_vals, 1 - level)
        reject = U_obs > crit

        U_obs_all.append(float(U_obs))
        crit_all.append(float(crit))
        reject_all.append(bool(reject))

        print(f"[Run {r+1:03d}] U={float(U_obs):.6f}  "
              f"crit(1-{level:.2f})={float(crit):.6f}  reject={bool(reject)}")

        # Für die ersten 5 Replikationen: Histogramm + vertikale Linie bei U(obs)
        if r < 5:
            plt.figure(figsize=(7, 4))
            plt.hist(np.array(T_b_vals), bins=30, edgecolor='black', alpha=0.8, density=True)
            plt.axvline(float(U_obs), linestyle='-', linewidth=2, label=f"U (obs) = {float(U_obs):.4f}")
            plt.title(f"Bootstrap (tilde-KSD, U) estimated theta — Run {r+1}  (n={n}, B={B}, μ={loc}, σ={scale})")
    ...
    U_obs_all = jnp.array(U_obs_all)
    crit_all = jnp.array(crit_all)
    reject_all = jnp.array(reject_all)

    print("\n=== Summary (U-only) ===")
    print(f"α = {level:.2f}")
    print(f"Mean threshold: {float(jnp.mean(crit_all)):.6f} "
          f"(Std: {float(jnp.std(crit_all)):.6f})")
    print(f"Meam U(obs): {float(jnp.mean(U_obs_all)):.6f} "
          f"(Std: {float(jnp.std(U_obs_all)):.6f})")
    print(f"Reject-Rate: {float(jnp.mean(reject_all)):.3f}")

    return U_obs_all, crit_all, reject_all

"""U_all, crit_all, reject_all = run_u_bootstrap_experiment(
    rng=key,
    n=500,
    runs=100,
    B=200,
...
# --- Use TRUE theta everywhere (no estimation) -------------------------------

THETA_TRUE = jnp.array([0.0, 1.0])  # (mu, sigma) for N(0,1)

def bootstrap_u_true_values(
    rng: jax.Array,
    ys: jax.Array,
    kernel: Kernel,
    null_class: type[UnnormalizedDist],
    B: int,
    theta_true: jax.Array = THETA_TRUE,
) -> jax.Array:
    """
    Your tilde-KSD bootstrap with *known* theta.
    Since theta_star == theta_hat == theta_true, the gradient correction is zero
    and the bootstrap statistic reduces to U*_h at theta_true.
    """
    score_fn = null_class.score_with_params
    _h_fn = h_fn_from_score(score_fn, kernel)
    h_fn_true = jax.tree_util.Partial(_h_fn, theta_true)

    # (We still call tilde_ksd with equal thetas; grad term = 0.)
    grad_h_true = jax.tree_util.Partial(grad_h_fn, theta_true)
    ...
    def one_bootstrap(k: jax.Array) -> jax.Array:
        ...
        return tilde_ksd(
            ...
            h_fn_star=h_fn_true,
            grad_h_fn_star=grad_h_true,
            grad_h_fn_hat=grad_h_true,
        )

    keys = jax.random.split(rng, B)
    return batch_vmap(one_bootstrap, keys, batch_size=10, progress=False)


def run_u_bootstrap_experiment_true(
    rng: jax.Array,
    *,
    n: int = 500,
    runs: int = 100,
    ...
    """
    For each run:
      - sample Y ~ null_class(loc, scale)
      - compute U_obs with TRUE theta
      - build bootstrap null with TRUE theta
      - record critical value and decision
      - plot the bootstrap histogram + U_obs for the first 5 runs
    """
    U_obs_all, crit_all, reject_all = [], [], []
    ...
    for r in range(runs):
        ...
        # Bootstrap null at true theta
        T_b_vals = bootstrap_u_true_values(k_boot, ys, kernel, null_class, B, theta_true)

        # Observed U at true theta
        U_obs = u_stat(kernel, null_class.score_with_params, theta_true, ys)

        # Critical value and decision (right-tailed)
        crit = jnp.quantile(T_b_vals, 1 - level)
        reject = U_obs > crit
        ...
        p_hat = float((T_b_vals >= U_obs).mean())
        print(f"[Run {r+1:03d}] U={float(U_obs):.6f}  crit(1-{level:.2f})={float(crit):.6f}  "
              f"p≈{p_hat:.3f}  reject={bool(reject)}")

        if r < 5:
            plt.figure(figsize=(7, 4))
            plt.hist(np.array(T_b_vals), bins=30, edgecolor='black', alpha=0.8, density=True)
            plt.axvline(float(U_obs), linestyle='-', linewidth=2, label=f"U (obs) = {float(U_obs):.4f}")
            plt.axvline(float(crit), linestyle=':', linewidth=2, label=f"crit = {float(crit):.4f}")
            plt.title(f"Bootstrap (tilde-KSD, U; true θ) — Run {r+1}  (n={n}, B={B}, μ={loc}, σ={scale})")
            plt.xlabel("Bootstrap-Statistic"); plt.ylabel("Dichte")
    ...
    print("\n=== Summary (U-only, true θ) ===")
    print(f"α = {level:.2f}")
    print(f"Mean threshold: {float(jnp.mean(crit_all)):.6f} (Std: {float(jnp.std(crit_all)):.6f})")
    print(f"Mean U(obs):   {float(jnp.mean(U_obs_all)):.6f} (Std: {float(jnp.std(U_obs_all)):.6f})")
    print(f"Reject-Rate:   {float(jnp.mean(reject_all)):.3f}")

    return U_obs_all, crit_all, reject_all

U_all, crit_all, reject_all = run_u_bootstrap_experiment_true(
    rng=key,
    n=500,
    runs=100,
    B=200,
