...
for i in tqdm(range(R)):
    ...
    Tn = u_stat(kernel, dist_class.score_with_params, theta_hat, ys)
...
plt.title("Empirical Distribution of n * KSD (U-statistic)")
plt.xlabel("n * KSD")
plt.ylabel("Density")
plt.grid(True)
plt.tight_layout()
...
for i in tqdm(range(R)):
    rng, key = jax.random.split(rng)
    ys = null_dist.sample(key, n)
    Tn = u_stat(kernel, null_dist.score_with_params, true_theta, ys)
