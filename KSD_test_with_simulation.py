...
#### SIMULATION of figure
# Set up
key = jax.random.PRNGKey(0)
n_values = [10, 25, 50, 100, 250]
B = 50       # number of bootstrap samples
R = 100       # number of test repetitions per n
...
# Loop over n
for n in tqdm(n_values, desc="Evaluating type I error rates"):
    reject_count_bootstrap = 0
    reject_count_wild = 0
    for _ in range(R):
        key, subkey = jax.random.split(key)
        # Generate data under the null
        X = Gaussian(2.0, 1.5).sample(subkey, n=n)

        # Our bootstrap
        res_bootstrap = KSD_test(
            rng=subkey,
            X=X,
            B=B,
            level=level,
            kernel=GaussianKernel(l=1.0),
            null_class=Gaussian,
            estimator=KSDAnalyticEstimator(GaussianKernel(l=1.0), Gaussian)
        )
        reject_count_bootstrap += int(res_bootstrap["reject_null"])

        # Wild bootstrap
        res_wild = wild_bootstrap_test(
            rng=subkey,
            ys=X,
            estimator=KSDAnalyticEstimator(GaussianKernel(l=1.0), Gaussian),
            test_statistic=KSDStatistic(GaussianKernel(l=1.0), Gaussian),
            n_bootstrap_samples=B,
            level=level,
        )
        reject_count_wild += int(res_wild.reject_null)

    type1_bootstrap.append(reject_count_bootstrap / R)
    type1_wild.append(reject_count_wild / R)

# Plotting
plt.figure(figsize=(6, 4))
plt.plot(n_values, type1_bootstrap, 'r+-', label='normal')
plt.plot(n_values, type1_wild, 'gP-', label='wild')
plt.axhline(y=level, linestyle='--', color='black')
plt.xlabel("number of observations, n")
plt.ylabel("type I error rate")
plt.title("Type I error rate under $H_0^C$ for the KSD test")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()