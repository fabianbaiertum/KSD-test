...
def rejection_rate_over_n_runs(
    *,
    ...
    scale: float = 2.0,
    runs: int = 20,
    kernel: Kernel = GaussianKernel(l=1.0) #Optional[Kernel] = None, #Kernel = GaussianKernel(l=1.0),  #TODO None only for the median heuristic, as it will be set inside the function
):
    ...
    for rep in range(runs):
        ...
        # --- add these two lines ---  #TODO only for the median heuristic
        #l = median_lengthscale(X)
        #kern = GaussianKernel(l=l) if kernel is None else kernel
        # ---------------------------
...
#rej_u, rate_u = rejection_rate_over_n_runs(mode="tilde_u")  #TODO uncomment
...
# === run it ===
plot_type1_vs_n_for_B_grid(
    n_values=(200, 300, 400, 500, 600),
    B_values=(100, 150, 200, 250),
    runs=100,         # bump this up if you want tighter error bars
    level=0.05,
    seed=4,
    loc=0.0,
    scale=2.0,
